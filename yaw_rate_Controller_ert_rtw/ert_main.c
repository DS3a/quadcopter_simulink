...
#include "yaw_rate_Controller.h"       /* Model header file */
...
void rt_OneStep(void)
{
  ...
  /* Check for overrun */
  if (OverrunFlag) {
    rtmSetErrorStatus(yaw_rate_Controller_M, "Overrun");
    return;
  }
...
int_T main(int_T argc, const char *argv[])
{
  ...
  fflush((NULL));
  while (rtmGetErrorStatus(yaw_rate_Controller_M) == (NULL)) {
    /*  Perform application tasks here */
  }

  /* Terminate model */
  yaw_rate_Controller_terminate();
  return 0;
}
